...
/*   Updated: 2022/11/21 21:26:37 by hdoo             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_move_right(t_world *world)
{
	...
	p = &world->player;
	if (!world->map[(int)(p->pos.y)][(int)(p->pos.x - p->dir.y * p->move_speed)])
		p->pos.x += -p->dir.y * p->move_speed;
	if (!world->map[(int)(p->pos.y + p->dir.x * p->move_speed)][(int)(p->pos.x)])
...
void	ft_move_backward(t_world *world)
{
	...
	p = &world->player;
	if (!world->map[(int)(p->pos.y)][(int)(p->pos.x - p->dir.x * p->move_speed)])
		p->pos.x -= p->dir.x * p->move_speed;
	if (!world->map[(int)(p->pos.y - p->dir.y * p->move_speed)][(int)(p->pos.x)])
